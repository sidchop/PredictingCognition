...
# In[4]:
# Model options
ind=ind #argv1 0=hcpep; 1=tcp; 2=cnp
perms = perms # argv2 set up number of splits for outer loop
...
kf_pearson_results = []
kf_COD_results = []
kf_Haufe_results  = np.zeros(((87571), perms))
...
for k in range(perms):
    ...
    trainLoader = DataLoader(dset_train,batch_size=batch_size,shuffle=False,num_workers=1)
    
    y_dummy = np.zeros(y_test.shape)
    dset_test = multi_task_dataset(x_test, y_dummy, True)
    testLoader = DataLoader(dset_test,batch_size=batch_size,shuffle=False,num_workers=1)
    ...
    for s in range(y_train.shape[1]):
        ...
        #y_test_final[:,s], _ = stacking_nulls(y_train_pred, y_test_pred, y_train[:,s],  alpha=rand_alpha)
        y_test_final[:,s], _, _ = stacking(y_train_pred, y_test_pred, y_train[:,s])
        y_train_haufe[:,s], _,_ = stacking(y_train_pred, y_train_pred, y_train[:,s])
	#y_train_haufe[:,s], _ = stacking_nulls(y_train_pred, y_train_pred, y_train[:,s], alpha=rand_alpha)                                                                        
    
    #  Compute H-tranformed feature weights
    kf_Haufe_results[:,k] = covariance_rowwise(x_train, y_train_haufe) 
...
kf_Haufe_results = pd.DataFrame(kf_Haufe_results)

output_path = '/gpfs/milgram/project/holmes/sc2998/MM/analysis/output/MM_stacking/nulls/'
