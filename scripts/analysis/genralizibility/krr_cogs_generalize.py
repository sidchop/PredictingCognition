...
# z-score FC values
fc_data_hcpep = zscore(fc_data_hcpep)
fc_data_tcp = zscore(fc_data_tcp)
fc_data_cnp= zscore(fc_data_cnp)
...
for i in range(3):
    for j in range(3):
        ...
        for p in range(y_input.shape[1]):
            ...
            obs_pred.to_csv(os.path.join(path_repo, 'output/generalizability/krr_train_'+names[i]+'_test_'+ names[j] + '.csv'), 
                                   sep=' ',
                                   index=False,
                                   header=False)
...
np.savetxt(os.path.join(path_repo, 'output/generalizability/krr_generalizability_matrix.txt'), cor_out)
np.savetxt(os.path.join(path_repo, 'output/generalizability/krr_generalizability_matrix_COD.txt'), COD_out)
