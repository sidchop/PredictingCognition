...
make_chord <- function(data=NULL, 
  ...
  outer_labels = c('TempPar','Default','Control','Limbic','VentAttn','DorsAttn','SomMot','Visual','Sub')
  ...
  circos.clear()
  circos.par("start.degree"= 90,"gap.after"= gap_vec, unit.circle.segments= 10000, canvas.ylim=c(-1.2,1.2))
  ...
  if  (link_colorscale == 'bkr'){
    link_colors = scan("~/Dropbox/Sid/python_files/metamatching/scripts/visualisation/bkr_colorscale.txt", what="character", sep="\n")
    bkr = function(vector){
